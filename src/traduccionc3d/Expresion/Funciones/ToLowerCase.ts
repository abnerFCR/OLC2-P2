...
export class ToLowerCase extends Expresion {
    ...
    compilar(entorno: Entorno) {
        ...
        if (variable.tipo.nombreTipo != Types.STRING) {
            throw new Error_(this.linea, this.columna, 'Semantico', 'La funcion toUpperCase solo se puede aplicar a tipo de dato string');
        }
        ...
        if (simbolo?.isGlobal) {
            ...
            generador.addCall('nativa_to_lowerCase');
